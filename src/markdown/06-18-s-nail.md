...
みどり（通常）：
  前回は MTA である Postfix を設定して起動したよ！
しげる（通常）：
  メールクライアントソフトを「MUA(mail user agent)」とよぶよ
  Linux の MUA は伝統的に `mail`, `mailx`, `nail` などなんだ
  今回はこれらの操作を引き継いだ MUA の S-nail を使ってメールを送受信してみよう！
...
しげる（通常）：
  `数字` を入れるとその番号のメールが表示されるよ
  何も入力せず Enter を押すと今見ているメール(`▸`) を表示するんだ
みどり（通常）：
  じゃあ　Enter だけ入力するね
...
みどり（通常）：
  `-e` は全て既読でもメールがあれば成功になっちゃうんだね
  新着や未読メールがあることを知りたい場合はどうすればいいのかな？
しげる（通常）：
  メール一覧の出力から判断しよう
  以下のようなコマンドはどうかな？
